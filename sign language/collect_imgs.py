...
#ret: ret is used to check whether the webcam is available or not (i.e.) if we open the camera application while running a OpenCV program, ret will identifies that the webcam is not available now so it will returns False and through an error.

#frame: frame is used to store the array values of respective pixels of the video

#'frame': A string representing the name of the window in which image to be displayed.
#frame: It is the image that is to be displayed.
